...
import { VENDOR_ADDITIONAL_CONTENT_20260305020000 } from './data/vendor-additional-content-20260305020000.data';
...
export class VendorAdditionalContentImport20260305020000
  implements MigrationInterface
{
  name = 'VendorAdditionalContentImport20260305020000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const payload of VENDOR_ADDITIONAL_CONTENT_20260305020000) {
      const vendorId =
  ...
  private async upsertCaseStudies(
    ...
    const existingByStudyUrl = new Map<string, ExistingCaseStudyRow>();
    const existingBySourceUrl = new Map<string, ExistingCaseStudyRow>();
    ...
    for (const row of existingRows) {
      ...
      const studyUrlKey = this.normalizeUrlKey(row.study_url);
      if (studyUrlKey && !existingByStudyUrl.has(studyUrlKey)) {
        existingByStudyUrl.set(studyUrlKey, row);
      }

      const sourceUrlKey = this.normalizeUrlKey(row.source_url);
      if (sourceUrlKey && !existingBySourceUrl.has(sourceUrlKey)) {
        existingBySourceUrl.set(sourceUrlKey, row);
      }
    }
    ...
    for (const caseStudy of normalizedCaseStudies) {
      ...
      const studyUrlKey = this.normalizeUrlKey(caseStudy.studyUrl);
      const sourceUrlKey = this.normalizeUrlKey(caseStudy.sourceUrl);

      const existing =
        (studyUrlKey ? existingByStudyUrl.get(studyUrlKey) : undefined) ||
        (sourceUrlKey ? existingBySourceUrl.get(sourceUrlKey) : undefined) ||
      ...
      if (existing) {
        ...
        const mergedStudyUrlKey = this.normalizeUrlKey(mergedRow.study_url);
        const mergedSourceUrlKey = this.normalizeUrlKey(mergedRow.source_url);

        if (mergedTitleKey) {
          existingByTitle.set(mergedTitleKey, mergedRow);
        }
        if (mergedStudyUrlKey) {
          existingByStudyUrl.set(mergedStudyUrlKey, mergedRow);
        }
        if (mergedSourceUrlKey) {
          existingBySourceUrl.set(mergedSourceUrlKey, mergedRow);
        }
      ...
      if (studyUrlKey) {
        existingByStudyUrl.set(studyUrlKey, insertedRow);
      }
      if (sourceUrlKey) {
        existingBySourceUrl.set(sourceUrlKey, insertedRow);
      }
  ...
  private normalizeCaseStudyKey(caseStudy: {
    title: string;
    studyUrl: string | null;
    sourceUrl: string | null;
  }): string {
    const studyUrlKey = this.normalizeUrlKey(caseStudy.studyUrl);
    if (studyUrlKey) {
      return `study-url::${studyUrlKey}`;
    }

    const sourceUrlKey = this.normalizeUrlKey(caseStudy.sourceUrl);
    if (sourceUrlKey) {
      return `source-url::${sourceUrlKey}`;
    }
